...
que faltam em todas as Administrações do Tipo C.
...
# Os 2 Administradores Municipais Adjuntos do Tipo C
ADJUNTOS_TIPO_C = [
    {
...
def adicionar_adjuntos():
    ...
    administrações do Tipo C.
    """
    # Buscar todas as administrações do Tipo C
    administracoes_tipo_c = Administracao.objects.filter(tipo_municipio='C')
    
    print(f"\n📊 Encontradas {administracoes_tipo_c.count()} administrações do Tipo C\n")
    
    total_criados = 0
    total_existentes = 0
    
    for admin in administracoes_tipo_c:
        print(f"\n{'='*60}")
        print(f"🏛️  {admin.nome} ({admin.provincia})")
        print(f"{'='*60}")
        
        for adjunto in ADJUNTOS_TIPO_C:
...
if __name__ == '__main__':
    print("="*70)
    print("ADICIONAR ADMINISTRADORES MUNICIPAIS ADJUNTOS - TIPO C")
